...
- test-split image-level diagnostics over MVTec AD bottle test records;
- image-level ROC AUC from the max patch score;
- defect-type score summaries and top-patch mask-hit diagnostics;
...
probe and should not be described as causal proof. The benchmark panel is a
local report diagnostic: image-level max-patch ROC AUC plus top-patch mask
overlap. It is not an official PatchCore benchmark reproduction or
benchmark-grade pixel-level AUROC. When `--coreset-size` is used,
nearest-normal patches shown in the report are retrieved from the retained
coreset.
...
Use `--benchmark-limit` when a quick smoke run should score only the first
subset of test records. Omit it for the full local MVTec AD bottle test split.
...
and now adds a narrow front-label template comparator to show what a
component-aware packaging rule can express that PatchCore distances cannot.
